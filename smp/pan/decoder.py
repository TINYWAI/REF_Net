...
class GAUScseBlock(nn.Module):
    def __init__(
        ...
        super(GAUScseBlock, self).__init__()
        ...
        self.sSE = nn.Sequential(nn.Conv2d(out_channels, 1, 1), nn.Sigmoid())
    ...
    def forward(self, x, y):
        ...
        x = self.conv2(x)
        z = x * self.cSE(y) + x * self.sSE(y_up)
        return z
...
class PANConvDecoder(nn.Module):
    ...
    def __init__(
        ...
        # self.fpa = FPABlock(in_channels=encoder_channels[-1], out_channels=decoder_channels)
        self.top_conv = ConvBnRelu(in_channels=encoder_channels[-1], out_channels=decoder_channels, kernel_size=3, stride=1, padding=1)
        self.gau3 = GAUBlock(in_channels=encoder_channels[-2], out_channels=decoder_channels, upscale_mode=upscale_mode)
        self.gau2 = GAUBlock(in_channels=encoder_channels[-3], out_channels=decoder_channels, upscale_mode=upscale_mode)
        self.gau1 = GAUBlock(in_channels=encoder_channels[-4], out_channels=decoder_channels, upscale_mode=upscale_mode)
    ...
    def forward(self, *features):
        bottleneck = features[-1]
        x5 = self.top_conv(bottleneck)         # 1/32
        x4 = self.gau3(features[-2], x5)  # 1/16
        x3 = self.gau2(features[-3], x4)  # 1/8
        x2 = self.gau1(features[-4], x3)  # 1/4

        return x2
class PANConvScseDecoder(nn.Module):
    ...
    def __init__(
        ...
        # self.fpa = FPABlock(in_channels=encoder_channels[-1], out_channels=decoder_channels)
        self.top_conv = ConvBnRelu(in_channels=encoder_channels[-1], out_channels=decoder_channels, kernel_size=3, stride=1, padding=1)
        self.gau3 = GAUScseBlock(in_channels=encoder_channels[-2], out_channels=decoder_channels, upscale_mode=upscale_mode)
        self.gau2 = GAUScseBlock(in_channels=encoder_channels[-3], out_channels=decoder_channels, upscale_mode=upscale_mode)
        self.gau1 = GAUScseBlock(in_channels=encoder_channels[-4], out_channels=decoder_channels, upscale_mode=upscale_mode)
...
class PANScseDecoder(nn.Module):
